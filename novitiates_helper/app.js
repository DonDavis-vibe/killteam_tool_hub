document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- I18N LOGIC ---
    let currentLang = localStorage.getItem('novitiates_lang') || 'en';
    ...
    document.getElementById('lang-en').addEventListener('click', () => {
        currentLang = 'en';
        localStorage.setItem('novitiates_lang', 'en');
        applyTranslations();
    });

    document.getElementById('lang-de').addEventListener('click', () => {
        currentLang = 'de';
        localStorage.setItem('novitiates_lang', 'de');
        applyTranslations();
    });
    ...
    function updateBuilderState() {
        ...
        Object.values(selectedOperatives).forEach(op => {
            const opId = op.opData.id;
            
            if (opId.includes('superior')) {
                hasSuperior = true;
            } else if (opId.includes('purgatus')) {
                purgatusCount++;
            } else if (!opId.includes('militant')) {
                // For all other non-militant specialists, extract the base name (e.g. condmenor_1 -> condemnor)
                const baseId = opId.split('_')[0];
                specialistCounts[baseId] = (specialistCounts[baseId] || 0) + 1;
                if (specialistCounts[baseId] > 1) {
                    isValid = false;
                }
            }
        });
        ...
        if (count === 10 && hasSuperior && isValid) {
            startBtn.disabled = false;
            startBtn.classList.add('glow-btn');
        } else {
    ...
    createTokenItem("Enemy Target 1 Grudge Tokens");
    createTokenItem("Enemy Target 2 Grudge Tokens");
