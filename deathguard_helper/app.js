document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateBuilderState() {
        ...
        let isValid = count === 6;
        if (!selectedOperatives['champion']) isValid = false;

        let gfCount = 0;
        if (selectedOperatives['heavy_gunner']) gfCount++;
        if (selectedOperatives['fighter']) gfCount++;
        if (selectedOperatives['gunner']) gfCount++;
        if (gfCount > 2) isValid = false;
    ...
    createTokenItem("Enemy Target 1 Poison Token");
    createTokenItem("Enemy Target 2 Poison Token");
